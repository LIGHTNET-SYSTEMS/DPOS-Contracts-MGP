...
namespace eosiosystem {
  ...
  /**
   ...
   void validate_team_holding_vesting( int64_t stake ) {
      const int64_t base_time = 1569888000; /// 2018-06-01
      const int64_t later_claimable = 36'000'000'0000ll;
      const int64_t initial_claimable = 4'000'000'0000ll;
      const int64_t max_claimable = initial_claimable + later_claimable;
      const int64_t claimable = initial_claimable + int64_t(later_claimable * double(current_time_point().sec_since_epoch() - base_time) / (6*seconds_per_half_year) );

      check( max_claimable - claimable <= stake, "team can only claim their tokens over 3 years" );
   }

   void validate_private_placement_vesting( int64_t stake ) {
      const int64_t base_time = 1569888000; /// 2018-06-01
      const int64_t later_claimable = 16'000'000'0000ll;
      const int64_t initial_claimable = 4'000'000'0000ll;
      const int64_t max_claimable = initial_claimable + later_claimable;
      const int64_t claimable = initial_claimable + int64_t(later_claimable * double(current_time_point().sec_since_epoch() - base_time) / (4*seconds_per_half_year) );

      check( max_claimable - claimable <= stake, "eosio.priv can only claim their tokens over 2 years" );
   }
   ...
   void system_contract::update_voting_power( const name& voter, const asset& total_update )
   {
      ...
      if( voter == "team"_n ) {
         validate_team_holding_vesting( voter_itr->staked );
      }

      if( voter == "priv"_n ) {
         validate_private_placement_vesting( voter_itr->staked );
      }
