...
BOOST_FIXTURE_TEST_CASE(producers_supply, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {

   const double usecs_per_halving  = 603 * 24 * 3600 * 1000000ll;
   const double secs_per_halving   = 603 * 24 * 3600;
   const double total_mining_amount = 300000000;
   ...
   transfer( config::system_account_name, "producvotera", core_sym::from_string("75000000.0000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvotera", core_sym::from_string("37500000.0000"), core_sym::from_string("37500000.0000")));
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      BOOST_REQUIRE_EQUAL(supply.get_amount() - initial_supply.get_amount(), 2487542400);

      produce_blocks(1);
      produce_block(fc::seconds(secs_per_halving - 24*3600 - 1));
      BOOST_REQUIRE_EQUAL(success(), push_action(N(defproducera), N(claimrewards), mvo()("owner", "defproducera")));

      supply  = get_token_supply();
      BOOST_REQUIRE_EQUAL(supply.get_amount() - initial_supply.get_amount(), 1499988067200);

      produce_blocks(1);
      produce_block(fc::seconds(secs_per_halving - 1));
      BOOST_REQUIRE_EQUAL(success(), push_action(N(defproducera), N(claimrewards), mvo()("owner", "defproducera")));

      supply  = get_token_supply();
      BOOST_REQUIRE_EQUAL(supply.get_amount() - initial_supply.get_amount(), 1499988067200*1.5);

      produce_blocks(1);
      produce_block(fc::seconds(secs_per_halving * 5 - 1));
      BOOST_REQUIRE_EQUAL(success(), push_action(N(defproducera), N(claimrewards), mvo()("owner", "defproducera")));

      supply  = get_token_supply();
      BOOST_REQUIRE_EQUAL(supply.get_amount() - initial_supply.get_amount(), 1499988067200*127/64);
      ...
      supply  = get_token_supply();
      BOOST_REQUIRE_EQUAL(supply.get_amount() - initial_supply.get_amount(), 1499988067200*4095/2048);
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   const int64_t usecs_per_halving  = 603 * 24 * 3600 * 1000000ll;
   const int64_t secs_per_halving   = 603 * 24 * 3600;
   const int64_t initial_mining_amount = 28791;
   const int64_t premine_amount = 200'000'000'0000;
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t time_since_activation = claim_time - microseconds_since_epoch_of_iso_string(global_state["thresh_activated_stake_time"]);
      const int64_t period = time_since_activation / usecs_per_halving;
      const int64_t remainder = double(time_since_activation % usecs_per_halving) / 1000'000ll;
      const int64_t total_supply = int64_t( double(2 * secs_per_halving * initial_mining_amount) * double(1 - double(std::pow(0.5, period))) + double(remainder * initial_mining_amount * std::pow(0.5, period)));
      const int64_t new_tokens = total_supply - initial_supply.get_amount() + premine_amount;

      BOOST_REQUIRE_EQUAL(0, initial_perblock_bucket);
      BOOST_REQUIRE_EQUAL(0, initial_pervote_bucket);
      BOOST_REQUIRE_EQUAL(new_tokens,
                          supply.get_amount() - initial_supply.get_amount());

      int64_t from_perblock_bucket = int64_t( double(new_tokens) / 4 ) ;
      int64_t from_pervote_bucket  = new_tokens - from_perblock_bucket;

      if (from_pervote_bucket >= 100) {
   ...
   // test stability over a year
   {
      ...
      BOOST_REQUIRE(500 * 10000 > int64_t(356 * 24 * 3600 * 2.4) - (supply.get_amount() - initial_supply.get_amount()));
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   for (const auto& v: voters) {
      ...
      transfer( config::system_account_name, v, core_sym::from_string("25000000.0000"), config::system_account_name );
      BOOST_REQUIRE_EQUAL(success(), stake(v, core_sym::from_string("12500000.0000"), core_sym::from_string("12500000.0000")) );
   }
...

   {
      ...
      const int64_t remainder = double(time_since_activation % usecs_per_halving) / 1000'000ll;
      const int64_t total_supply = int64_t( double(2 * secs_per_halving * initial_mining_amount) * double(1 - double(std::pow(0.5, period))) + double(remainder * initial_mining_amount * std::pow(0.5, period)));
      const int64_t expected_supply_growth = total_supply - initial_supply.get_amount() + premine_amount;
      ...
      const int64_t expected_pervote_bucket  = expected_supply_growth - expected_perblock_bucket;
...

   {
      ...
      const int64_t expected_perblock_bucket = expected_supply_growth / 4 + initial_perblock_bucket;
      const int64_t expected_pervote_bucket  = expected_supply_growth - expected_perblock_bucket + initial_perblock_bucket + initial_pervote_bucket;
...

   {
      ...
      BOOST_REQUIRE_EQUAL(success(),
                          push_action(prod_name, N(claimrewards), mvo()("owner", prod_name)));
      BOOST_REQUIRE(100 <= get_balance(prod_name).get_amount());
      BOOST_REQUIRE_EQUAL(wasm_assert_msg("already claimed rewards within past day"),
                          push_action(prod_name, N(claimrewards), mvo()("owner", prod_name)));
   }
   ...
   // switch to new producer pay metric
   {
      ...
      BOOST_REQUIRE( 100 < from_pervote_bucket );
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, eosio_system_tester) try {
   ...
   transfer(config::system_account_name, "producvotera", core_sym::from_string("100000.0000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvotera", core_sym::from_string("35000.0000"), core_sym::from_string("35000.0000") ));
   BOOST_REQUIRE_EQUAL(success(), vote( N(producvotera), vector<account_name>(producer_names.begin(), producer_names.begin()+10)));
   ...
   transfer(config::system_account_name, "producvoterb", core_sym::from_string("60000000.0000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvoterb", core_sym::from_string("40000000.0000"), core_sym::from_string("20000000.0000")));
   transfer(config::system_account_name, "producvoterc", core_sym::from_string("30000000.0000"), config::system_account_name);
   BOOST_REQUIRE_EQUAL(success(), stake("producvoterc", core_sym::from_string("20000000.0000"), core_sym::from_string("10000000.0000")));
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "eosio", "alice1111111", core_sym::from_string("80000000.0000"), "eosio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("40000000.0000"), core_sym::from_string("40000000.0000") ) );
   //vote for producers
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   stake_with_transfer( config::system_account_name, "sam", core_sym::from_string( "35000000.0000" ), core_sym::from_string( "35000000.0000" ) );
   stake_with_transfer( config::system_account_name, "dan", core_sym::from_string( "20000000.0000" ), core_sym::from_string( "20000000.0000" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), bidname( "sam", "nofail", core_sym::from_string( "10.0000" ) )); // didn't increase bid by 10%
   produce_block( fc::days(1) );
   produce_block();
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   stake_with_transfer( config::system_account_name, "bob",  core_sym::from_string( "350000.0000" ), core_sym::from_string( "350000.0000" ) );
   stake_with_transfer( config::system_account_name, "carl", core_sym::from_string( "350000.0000" ), core_sym::from_string( "350000.0000" ) );
   ...
   stake_with_transfer( config::system_account_name, "alice", core_sym::from_string( "50000000.0000" ), core_sym::from_string( "50000000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( vote_producers_in_and_out, eosio_system_tester ) try {
   ...
   for (const auto& v: voters) {
      transfer( config::system_account_name, v, core_sym::from_string("27000000.0000"), config::system_account_name );
      BOOST_REQUIRE_EQUAL(success(), stake(v, core_sym::from_string("12500000.0000"), core_sym::from_string("12500000.0000")) );
   }
   ...
   // give a chance for everyone to produce blocks
   {
      ...
      // printf("%ld\n%ld\n", all_21_produced, rest_didnt_produce);
      BOOST_REQUIRE(all_21_produced && rest_didnt_produce);
   }
...
BOOST_FIXTURE_TEST_CASE( team_vesting, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), bidname( bob,   team, core_sym::from_string( "0.5000" ) ) );
   BOOST_REQUIRE_EQUAL( success(), bidname( alice, team, core_sym::from_string( "1.0000" ) ) );

   produce_block( fc::days(1) );

   create_accounts_with_resources( { team }, alice );

   const asset stake_amount = core_sym::from_string("40000000.0000");
   const asset half_stake   = core_sym::from_string("20000000.0000");
   const asset small_amount = core_sym::from_string("2000000.0000");
   issue_and_transfer( team, stake_amount + stake_amount + stake_amount, config::system_account_name );

   stake( team, team, half_stake, half_stake );

   BOOST_REQUIRE_EQUAL( stake_amount.get_amount(), get_voter_info( team )["staked"].as<int64_t>() );

   BOOST_REQUIRE_EQUAL( success(), unstake( team, team, small_amount, small_amount ) );

   produce_block( fc::days(4) );

   BOOST_REQUIRE_EQUAL( success(), push_action( team, N(refund), mvo()("owner", team) ) );

   BOOST_REQUIRE_EQUAL(( stake_amount.get_amount() - 2 * small_amount.get_amount() ),
                        get_voter_info( team )["staked"].as<int64_t>() );

   produce_block( fc::days( 364 ) );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("team can only claim their tokens over 3 years"),
                        unstake( team, team, half_stake - small_amount, half_stake - small_amount) );

   BOOST_REQUIRE_EQUAL( success(), vote( team, { }, N(proxyaccount) ) );
   BOOST_REQUIRE_EQUAL( success(), unstaketorex( team, team, half_stake - small_amount, half_stake - small_amount) );

   produce_block( fc::days(5) );
   produce_blocks(1);

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("team can only claim their tokens over 3 years"),
                        sellrex( team, get_rex_balance( team ) ) );

   BOOST_REQUIRE_EQUAL( success(), rentcpu( bob, bob, core_sym::from_string("10000.0000") ) );

   BOOST_REQUIRE_EQUAL( wasm_assert_msg("team sellrex orders should not be queued"),
                        sellrex( team, get_rex_balance( team ) ) );

   produce_block( fc::days( 2 * 364 ) );

   BOOST_REQUIRE_EQUAL( success(), sellrex( team, get_rex_balance( team ) ) );
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE( double(tokens_paid_for_ram.get_amount() - tokens_received_by_selling_ram.get_amount()) / tokens_paid_for_ram.get_amount() < 0.011 );
